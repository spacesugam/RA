...
class OpenAIService {
    constructor() {
        this.recentTopics = []; // Track recent topics
        this.maxTopicHistory = 20; // Remember last 20 topics
     ...
     */
    async generateBotRoast(topicData, allMessages, botUsername, opponentUsername, botSide, currentRound, lastOpponentMessage) {
...
Output ONLY the roast line.`;
        try {
            const completion = await this.openai.chat.completions.create({
                ...
                messages: [
                    {
                        role: 'system',
                        content: `You are ${botUsername}, an elite roast battler in pure roast mode. Be clever, concise, and brutally honest. Hit hard, keep rhythm tight, but strictly avoid hate speech, slurs, or real-world harm. One-liner preferred; two sentences only if it heightens the burn.`
                    },
     ...
     */
    async generateCategoryBasedTopic() {
    ...
    getFallbackTopic() {
        ...
        const availableTopics = this.fallbackTopics.filter(topic => this.isTopicUnique(topic.topic));
     ...
     */
    async judgeBattle(topicData, player1Messages, player2Messages, player1Username, player2Username, player1Side, player2Side) {
      ...
      `;
            ...
            const bonus = (difficulty) => difficulty > 6 ? 2 : difficulty > 4 ? 1 : 0;
    ...
    getFallbackJudgment(player1Username, player2Username) {
